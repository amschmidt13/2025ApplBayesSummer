...
code_micro<-df_ba_binomial$code_micro
obese<-as.numeric(df_ba_binomial$obesity)
followed<-df_ba_binomial$accompanieds
bfp<-df_ba_binomial$bfp
prop_illiterate<-df_ba_binomial$illit
avg_percapita_income<-as.numeric(df_ba_binomial$percapita_income)
health_teams<-df_ba_binomial$health_teams
...
hist(log(avg_percapita_income),prob=1)

obes_bahia<-data.frame(code_micro=code_micro,obese=obese,followed=followed,bfp=bfp,prop_illiterate=prop_illiterate,avg_percapita_income=avg_percapita_income,health_teams=health_teams)
...
modelBYM = stan("./BYM_Model_Binomial.stan", 
                ...
                warmup=2500, iter=15000, chains=2, thin = 5)
...
# Some maps
muBYM = data.frame(round(apply(samplesBYM$mu, 2, mean)))
colnames(muBYM)[1] = "Posterior"
muBYM$code_micro = df_ba$code_micro

posteriorBYM = left_join(muBYM, df_ba, by = "code_micro")
posteriorBYM$MuRate = (posteriorBYM$Posterior/posteriorBYM$pop0a4)*100
posteriorBYM$TrueRate = (posteriorBYM$intern_diarreia/posteriorBYM$pop0a4)*100


(mapTRUE = 
    ggplot() +
    geom_sf(data = posteriorBYM, aes(fill = TrueRate, geometry = geom)) +
    scale_fill_viridis_c(name = "Hosp. by 1000 children", na.value = "grey") +
    labs(title = "Observed data") + 
    theme_bw()) 

(mapBYM = 
    ggplot() +
    geom_sf(data = posteriorBYM, aes(fill = MuRate, geometry = geom)) +
    scale_fill_viridis_c(name = "Hosp. by 1000 children", na.value = "grey") +
    labs(title = "Posterior Means") + 
    theme_bw()) 

ggarrange(mapTRUE, mapBYM, align = "hv", ncol = 2, common.legend = T, 
          legend = "right")
